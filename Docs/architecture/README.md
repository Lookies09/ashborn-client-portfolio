...
> 현재 공개 코드는 이동 판정과 scan 진행을 분리합니다. 시작 위치와 대상 수를 캡처하고, `_scanInProgress`와 `_currentIndex`를 유지해 전체 대상 처리가 끝날 때까지 여러 frame에 걸쳐 scan을 계속합니다. 자세한 내용은 [기술 문서](../technical/README.md#1-enemy-multi-frame-scan-stopping-after-the-first-chunk)에 기록합니다.
